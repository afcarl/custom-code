...
for (i in 1:length(top20)) {
	...
	model <- glm(class ~ promoter_meth + gb_meth + expression, data=df_train,family="binomial")
	
	predictions_t <- predict(model,df_predict[Ts_toPredict,],"response")
	predictions_an <- predict(model,df_predict[ANs_toPredict,],"response")
