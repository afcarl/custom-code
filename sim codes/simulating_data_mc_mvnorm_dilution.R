...
sim.data <- mclapply(1:numJobs, mc.cores=47, mc.cleanup = TRUE, mc.preschedule = TRUE, function(i) {
	...
	if (n > 0) {
		...
		temp_counts <- temp[,1]
		temp_gb <- temp[,2]
		temp_gb <- transform(vector=temp_gb,targetMean=diff*2.5,targetSd=2.5*error_term)
		temp_pr <- temp[,3]
		temp_pr <- transform(vector=temp_pr,targetMean=-diff*2.5,targetSd=2.5*error_term)
	...
	if (n > 0) {
		...
		temp_counts <- trunc(transform(vector=temp_counts,targetMean=read_count,targetSd=0.125*read_count) + rnorm(n,0,8))
		...
		for (fake in 1:n) {
			temp_pr_cpg[fake,] <- rnorm(n=11,mean=temp_fake[fake,3],sd=2.5*error_term) + rnorm(11,0,error)
			temp_gb_cpg[fake,] <- rnorm(n=11,mean=temp_fake[fake,2],sd=2.5*error_term) + rnorm(11,0,error)
		}
