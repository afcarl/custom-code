...
sim.data <- mclapply(1:numJobs, mc.cores=47, mc.cleanup = TRUE, mc.preschedule = TRUE, function(i) {
	...
	delta_correlations <- sim.params[current_param,2]
	...
	Sigma <- matrix(c(3,1.5-3*delta_correlations,-1.5+3*delta_correlations,1.5-3*delta_correlations,3,-3,-1.5+3*delta_correlations,-3,3),3,3,byrow=TRUE)
	temp <- mvrnorm(n=100, c(0,0,0), Sigma)
	temp_counts <- temp[,1]
	temp_gb <- temp[,2]
	temp_gb <- transform(vector=temp_gb,targetMean=0,targetSd=1.5*error_m)
	temp_pr <- temp[,3]
	temp_pr <- transform(vector=temp_pr,targetMean=0,targetSd=1.5*error_m)
	temp_counts <- trunc(transform(vector=temp_counts,targetMean=read_count,targetSd=error_e*2) + rnorm(100,0,error_e))
	...
	for (t in 1:nrow(temp_t)) {
		temp_pr_cpg[t,] <- rnorm(n=11,mean=temp_t[t,3],sd=2*error_m) + rnorm(11,0,error_m/1.5)
		temp_gb_cpg[t,] <- rnorm(n=11,mean=temp_t[t,2],sd=2*error_m) + rnorm(11,0,error_m/1.5)
	}
