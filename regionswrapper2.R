                            ...
                            adjustpcut = NULL, betadiffcut = NULL) 
{
    ...
    list11 = c("TSS1500Ind", "TSS200Ind", "UTR5Ind", "EXON1Ind", 
               "GENEBODYInd", "UTR3Ind", "ISLANDInd", "NSHELFInd", "NSHOREInd", "SSHELFInd", "SSHOREInd")
    for (i in 1:11) {
        ...
        eval(parse(text = paste(list11[i], "test=as.data.frame(testfunc2(eset,concov = concov,testmethod=testmethod,Padj=Padj,groupinfo = groupinfo,paired = paired,gcase = gcase,gcontrol = gcontrol))", 
                                sep = "")))
