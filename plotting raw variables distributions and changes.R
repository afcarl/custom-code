...
plotter1 <- data.frame(metric=rep(density(mmatrix_BRCA_PROMOTER,from=-7,to=7)$x,2),density=c(
density(mmatrix_BRCA_PROMOTER[,ANs],from=-7,to=7)$y,
density(mmatrix_BRCA_PROMOTER[,Ts],from=-7,to=7)$y), type=c(rep("ANs_all",512),rep("Ts_all",512)))
#ggplot(plotter1,aes(x=B_value,y=density)) + geom_line(aes(colour=type),lwd=0.5) + scale_colour_manual(values=cb_palette)
plotter2 <- data.frame(metric=rep(density(mmatrix_BRCA_BODY,from=-7,to=7)$x,2),density=c(
density(mmatrix_BRCA_BODY[,ANs],from=-7,to=7)$y,
density(mmatrix_BRCA_BODY[,Ts],from=-7,to=7)$y), type=c(rep("ANs_all",512),rep("Ts_all",512)))
#ggplot(plotter2,aes(x=B_value,y=density)) + geom_line(aes(colour=type),lwd=0.5) + scale_colour_manual(values=cb_palette)
plotter3 <- data.frame(metric=rep(density(cpm_BRCA_plusOne[,ANs],from=0,to=250)$x,2),density=c(density(cpm_BRCA_plusOne[,ANs],from=0,to=250)$y,density(cpm_BRCA_plusOne[,Ts],from=0,to=250)$y), type=c(rep("ANs_all",512),rep("Ts_all",512)))
ggplot(plotter3,aes(x=metric,y=density)) + theme_bw() + geom_line(aes(colour=type),lwd=0.5) + scale_colour_manual(values=cb_palette) + scale_x_log10()
...
ggplot(plotter4,aes(x=metric,y=density)) + theme_bw() + geom_line(aes(colour=type),lwd=0.5) +facet_wrap(facets=~element,scales="free") +ggtitle("Global distributions of variables")+ theme(legend.position="bottom") + scale_colour_manual(values=cb_palette)
